...
class ServicePage(Page):
    ...
    internal_page = models.ForeignKey(
        ...
        null = True,
        related_name = '+',
        help_text = 'Select a internal Wagtail page',
    ...
    def clean(self):
        ...
        if not self.internal_page and not self.external_page:
            # Any of the fields are not filled out
            raise ValidationError({
                'internal_page': ValidationError("You must always select a page OR enter an external URL"),
                'external_page': ValidationError("You must always select select a page OR enter an external URL"),
