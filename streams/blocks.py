...
class TitleBlock(blocks.StructBlock):
    ...
    class Meta:
        ...
        icon = "edit"
        label = "Title"
        help_text = "Centered text to be display on the page"

class LinkValue(blocks.StructValue):
    # Additional logic for our link
...
class Link(blocks.StructBlock):
    ...
    def clean(self, value):
        ...
        if internal_page and external_link:
            ...
            errors["internal_page"] = ErrorList(["Please select a pae or enter a URL for one of these options."])
            errors["external_link"] = ErrorList(["Please select a pae or enter a URL for one of these options."])
...
class ImageAndTextBlock(blocks.StructBlock):
    image = ImageChooserBlock(
        help_text='Image will be automagically cropped 786px by 552px',
    )
    image_alignment = RadioSelectBlock(
        ...
        default = 'left',
        help_text='Image on the left with text on the right. Or image opn the right with text on the left',
...
class CallToActionBlock(blocks.StructBlock):
    title = blocks.CharBlock(
        max_length = 200,
        help_text = 'Max length of 200 characeters.',
    ) 
    link = Link()
...
class PricingTableBlock(TableBlock):
    ...
    class Meta:
        ...
        label = "Pricing Table"
        icon = "table"
        help_text = "Your pricing tabled should always have 4 columns"
